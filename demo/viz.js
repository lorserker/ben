...
class PlayInfo {
    ...
    render(element, index) {
        ...
        let html = "<h3>Play</h3><br> " + this.data["card"] +  (this.data["who"] !== undefined ? " selected by " + this.data["who"]: "")  + "<br/><br/>"
        ...
        if ("candidates" in this.data && this.data.candidates.length > 0) {
            ...
            for (const element of this.data.candidates) {
                ...
                if ("expected_tricks" in candidate && candidate['expected_tricks'] !== -1) {
                ...
                if ("expected_tricks_dd" in candidate && candidate['expected_tricks_dd'] !== -1) {
        ...
        if ("samples" in this.data) {
            html += generateSamplesTable(this.data.samples, this.data["card"]);
        }
