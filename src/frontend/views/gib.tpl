...
<head>
	...
	<script>
		...
		function loadXMLDoc() {
			let cachedData = localStorage.getItem('bidData_' + biddingsequence);
			...
			if (cachedData) {
				...
				xmlhttp.onreadystatechange = function () {
					if (this.readyState == 4 && this.status == 200) {
						...
						localStorage.setItem('bidData_' + biddingsequence, data); // Cache the response
						let parser = new DOMParser();
...
<body onload="loadXMLDoc()">
    ...
    <script>
		...
		function clearCache() {
			...
			// localStorage.removeItem('bidData_' + bid); 
