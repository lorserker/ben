...
class Sample:
    ...
    def __init__(self, lead_accept_threshold, bidding_threshold_sampling, play_accept_threshold,
                 bid_accept_play_threshold, sample_hands_auction):
    ...
    def sample_cards_auction(self, n_samples, n_steps, auction, nesw_i, hand, vuln, bidder_model, binfo, ns, ew):
        ...
        A_lho = binary.get_auction_binary(
            n_steps, auction, (nesw_i + 1) % 4, hand, vuln, ns, ew)
        A_pard = binary.get_auction_binary(
            n_steps, auction, (nesw_i + 2) % 4, hand, vuln, ns, ew)
        A_rho = binary.get_auction_binary(
            n_steps, auction, (nesw_i + 3) % 4, hand, vuln, ns, ew)
        ...
        lho_pard_rho = self.sample_cards_vec(
        ...
        lho_actual_bids = bidding.get_bid_ids(
            auction, (nesw_i + 1) % 4, n_steps)
        ...
        pard_actual_bids = bidding.get_bid_ids(
            auction, (nesw_i + 2) % 4, n_steps)
        ...
        rho_actual_bids = bidding.get_bid_ids(
            auction, (nesw_i + 3) % 4, n_steps)
        ...
        accepted_samples = lho_pard_rho[min_scores > accept_bidding_threshold]
        while accepted_samples.shape[0] < 20:
            accept_bidding_threshold *= 0.9
            accepted_samples = lho_pard_rho[min_scores >
                                            accept_bidding_threshold]

        return accepted_samples

    def shuffle_cards_bidding_info(self, n_samples, binfo, auction, hand, vuln, known_nesw, h_1_nesw, h_2_nesw,
                                   visible_cards, hidden_cards, cards_played, shown_out_suits, ns, ew):
    ...
    def init_rollout_states(self, trick_i, player_i, card_players, player_cards_played, shown_out_suits, current_trick,
                            n_samples, auction, hand, vuln, models, ns, ew):
        ...
        for i in range(2):
            for j in range(4):
                if np.round(c_shp[i, j] >= 5):
                    accept_shp &= np.sum(
                        states[[hidden_1_i, hidden_2_i][i]][:, 0, (j * 8):((j + 1) * 8)], axis=1) >= np.round(
                        c_shp[i, j]) - 1
        ...
        if trick_i <= 9:
            for p_i in [hidden_1_i, hidden_2_i]:
                ...
                cards_played_prev_tricks = player_cards_played[p_i][1:
                                                                    trick_i] if p_i == 0 else player_cards_played[p_i][
                                                                                              :trick_i]
