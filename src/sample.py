...
def sample_cards_auction(n_samples, n_steps, auction, nesw_i, hand, vuln, bidder_model, binfo_model):
    ...
    A_lho = binary.get_auction_binary(n_steps, auction, (nesw_i + 1) % 4, hand, vuln)
    A_pard = binary.get_auction_binary(n_steps, auction, (nesw_i + 2) % 4, hand, vuln)
    A_rho = binary.get_auction_binary(n_steps, auction, (nesw_i + 3) % 4, hand, vuln)
...
def shuffle_cards_bidding_info(n_samples, binfo, auction, hand, vuln, known_nesw, h_1_nesw, h_2_nesw, visible_cards, hidden_cards, cards_played, shown_out_suits):
    ...
    f_trans_hcp = lambda x: 4 * x + 10
    f_trans_shp = lambda x: 1.75 * x + 3.25
...
def get_bid_scores(nesw_i, auction, vuln, hand, bidder_model):
    n_steps = 1 + len(auction) // 4
