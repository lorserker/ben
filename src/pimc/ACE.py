...
class ACEDLL:
            ...
            with cls._lock:  # Ensure only one thread can enter this block at a time
                if cls._dll_loaded is None:  # Double-checked locking
                    try:
                        ...
                        from ACE import Game, Engine, GameOptions, ConstraintSet, Player, Contract
                        ...
                        print("*****************************************************************************")
                        print("Error: Unable to load ACEDLL.dll. Make sure the DLL is in the ./bin directory")
    ...
    def version(self):
        dll = ACEDLL.get_dll()  # Retrieve the loaded DLL classes through the singleton
        PIMC = dll["PIMC"]
        return PIMC().version()
    ...
    def reset_trick(self):
        dll = ACEDLL.get_dll()  # Access the loaded DLL singleton
