        ...
        for trick_i in range(12):
            for player_i in map(lambda x: x % 4, range(leader_i, leader_i + 4)):
                ...
                if isinstance(card_players[player_i], bots.CardPlayer):
                    rollout_states = sample.init_rollout_states(trick_i, player_i, card_players,
                     player_cards_played, shown_out_suits, current_trick, 200, self.padded_auction,
                     card_players[player_i].hand.reshape((-1, 32)), self.vuln, self.models)
