"""
ACE-MCTS (Monte Carlo Tree Search) wrapper for BEN - Defender Play

This module provides an interface to the Brill.Ace.Mcts library for defensive play,
...
logger = logging.getLogger("ace_mcts_def")
...
class ACEMCTSDefDLL:
    ...
    def __init__(self, models, northhand, southhand, contract, is_decl_vuln, player_i, sampler, verbose):
        """
        Initialize ACE-MCTS defender play engine.
        ...
        """
        dll = ACEMCTSDefDLL.get_dll(verbose)
        if dll is None:
        ...
        if self.dds_library == 'haglund':
            if self.HaglundSolver.IsAvailable:
                ...
                if verbose:
                    print(f"ACE-MCTS Defender using Bo Haglund's dds.dll")
            else:
                self.solver = self.BcalcddsSolver()
                if verbose:
                    print(f"ACE-MCTS Defender: dds.dll not available, falling back to libbcalcdds")
        else:
            self.solver = self.BcalcddsSolver()
            if verbose:
                print(f"ACE-MCTS Defender using libbcalcdds")
    ...
    def version(self):
        try:
            dll = ACEMCTSDefDLL.get_dll(False)
    ...
    def set_shape_constraints(self, min_declarer, max_declarer, min_partner, max_partner, quality):
        """Set shape constraints for declarer and partner"""
    ...
    def update_constraints(self, playedBy, real_card):
        ...
        if self.player_i == 0:
            # We are West: declarer=South(3), partner=East(2)
            ...
            # We are East: declarer=South(3), partner=West(0)
            if playedBy == 3:  # Declarer (South)
                idx = suit * 2
                self.declarer_constraints[idx] = max(0, self.declarer_constraints[idx] - 1)
                self.declarer_constraints[idx + 1] = max(0, self.declarer_constraints[idx + 1] - 1)
    ...
    def set_card_played(self, card52, playedBy, openinglead):
        ...
        if self.verbose:
            print(f"Setting card {real_card} played by {playedBy} for ACE-MCTS Defender")
    ...
    def nextplay(self, player_i, shown_out_suits, missing_cards):
        """Find the best card to play using ACE-MCTS engine for defender"""
        t_start = time.time()
        ...
        try:
            ...
            logger.info("ACE-MCTS-Def Request: PBN=%s Contract=%s Player=%s Played=%s Constraints=[Decl:%s Partner:%s] Depth=%s Confidence=%s Exploration=%s",
            ...
            try:
                ...
                logger.info("ACE-MCTS-Def Response: %s Iterations=%s Passed=%s Depth=%s Relaxation=%s Time=%.1fs",
            ...
            logger.error("ACE-MCTS-Def nextplay error: %s", e, exc_info=True)
            print(f'{Fore.RED}Error in ACE-MCTS Def nextplay: {e}{Fore.RESET}')
            traceback.print_exc()
            return {}

        if self.verbose:
            print(f"Returning {len(card_result)} from ACE-MCTS Def nextplay")
