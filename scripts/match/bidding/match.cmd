...
python auction.py --bidderNS=A.conf --bidderEW=A.conf --set=%boards% --search=EW --nntrust=%trust%  > .\%trust%\auctionsEW.json
python auction.py --bidderNS=A.conf --bidderEW=A.conf --set=%boards% --search=NS --nntrust=%trust%  > .\%trust%\auctionsNS.json

type ".\%trust%\auctionsNS.json" | python lead.py --bidder=A.conf > .\%trust%\leads1.json
type ".\%trust%\auctionsEW.json" | python lead.py --bidder=A.conf > .\%trust%\leads2.json
...
rem find and sum positive scores
powershell -Command "(Get-Content .\%trust%\imps.txt | ForEach-Object { $_ -as [double] }) | Where-Object { $_ -lt 0 } | Measure-Object -Sum | Select-Object -ExpandProperty Sum"

python printdeal.py %boards% %1