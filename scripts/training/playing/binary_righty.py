...
from lead_binary_util import DealMeta, seats, seat_index, suit_index_lookup, card_index_lookup_x, hot_encode_card, binary_hand
...
def binary_data(deal_str, outcome_str, play_str):
    ...
    for i, (on_lead, last_trick, cards_in, card_out) in enumerate(zip(on_leads, last_tricks, cards_ins, card_outs)):
        if i > 10:
            break
        #print(card_out)
        suit = "SHDC".index(card_out[0])
        #print(hands[me_i][suit])
        if not (card_out[1] in hands[me_i][suit]):
