...
y_shape = np.load(os.path.join(bin_dir, 'SHAPE.npy'), mmap_mode='r')

# Load scaled value targets (from your pkl→npy converter)
value_path = os.path.join(bin_dir, 'selfplay_value.npy')  # or 'value.npy' if you named it that
y_value = np.load(value_path).astype(np.float32)

# Ensure shape is (N, 1)
if y_value.ndim == 1:
    y_value = y_value.reshape(-1, 1)

# Align lengths: use the minimum of all four
n_examples = min(X_train.shape[0], y_train.shape[0], y_shape.shape[0], y_value.shape[0])

X_train = X_train[:n_examples]
y_train = y_train[:n_examples]
y_shape = y_shape[:n_examples]
y_value = y_value[:n_examples]

print("Using", n_examples, "examples with value targets")


n_shape = y_shape.shape[-1]
...
# Build model
# -------------------------
def build_model(input_shape, lstm_size, n_layers, n_bids, n_shape):
    ...
    # Policy head
    bid_outputs = layers.TimeDistributed(
        layers.Dense(n_bids, activation='softmax'),
        name='bid_output'
    )(shared)

    # Shape head
    shape_outputs = layers.TimeDistributed(
        layers.Dense(n_shape, activation='linear'),
        name='shape_output'
    )(shared)

    # Value head (scalar)
    final_state = shared[:, -1, :]
    value_hidden = layers.Dense(128, activation='relu')(final_state)
    value_output = layers.Dense(1, activation='tanh', name='value_output')(value_hidden)

    model = models.Model(
        inputs=inputs,
        outputs=[bid_outputs, shape_outputs, value_output]
    )
    ...
    model.compile(
        ...
        loss={
            'bid_output': 'categorical_crossentropy',
            'shape_output': 'mse',
...
def create_dataset():
    outputs = {
        'bid_output': y_train,
        'shape_output': y_shape,
        'value_output': y_value,
    }

    dataset = tf.data.Dataset.from_tensor_slices((X_train, outputs))
...
model = build_model((None, n_ftrs), lstm_size, n_layers, n_bids, n_shape)
...
class ResourceMonitor(tf.keras.callbacks.Callback):
    def on_epoch_end(self, epoch, logs=None):
        ...
        if physical_devices:
            ...
            print(f"Epoch {epoch + 1}: GPU monitoring skipped (CPU-only).")
