...
s_train = np.load(os.path.join(bin_dir, 's.npy'))
u_train = np.load(os.path.join(bin_dir, 'u.npy'))

n_examples = X_train.shape[0]
n_ftrs = X_train.shape[1] + s_train.shape[1]

input_dim_hands = X_train.shape[1]
input_dim_suit = s_train.shape[1]
...
print("Size input hands:        ", input_dim_hands)
print("Size input suit:         ", input_dim_suit)
print("Size output tricks:      ", output_dim_tricks)
print("-------------------------")
print("dtype X_train:           ", X_train.dtype)
print("dtype s_train:           ", s_train.dtype)
...
def build_model(input_shape, lstm_size, n_layers):
    ...
    model = models.Model(inputs=inputs, outputs=[tricks_output])
...
# Create datasets
    ...
    def data_generator(X, s, u):
        for xi, si,ui in zip(X, s, u):
            # Concatenate features x, y, and z
            concatenated_features = np.concatenate((xi, si), axis=0)
            yield (concatenated_features, {'tricks_output': ui})

    # Adjust the output_signature to include only tricks_output
    output_signature = (
    ...
    train_dataset = tf.data.Dataset.from_generator(
        lambda: data_generator(X_train, s_train, u_train),
