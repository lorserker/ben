...
if __name__ == '__main__':
    ...
    n1 = 0
    n2 = 0
    for i, (deal_str, outcome_str, auction_str, play_str) in enumerate(data_it):
    ...
    A = np.zeros((n1, n_steps, 2 + 1 + 4 + 32 + 3 * 40), dtype=np.float16)
    B = np.zeros((n1, 15), dtype=np.float16)
    X = np.zeros((n1, 42), dtype=np.float16)
    Y = np.zeros((n1, 32), dtype=np.float16)

    i = 0
    for j, (deal_str, outcome_str, auction_str, play_str) in enumerate(data_it_nt):
        d_meta = DealMeta.from_str(outcome_str)
        if d_meta.strain == "N":
    ...
    out_dir = './lead_keras_suit'

    print(f"Processing {n2} deals for suit")

    A = np.zeros((n2, n_steps, 2 + 1 + 4 + 32 + 3 * 40), dtype=np.float16)
    B = np.zeros((n2, 15), dtype=np.float16)
    X = np.zeros((n2, 42), dtype=np.float16)
    Y = np.zeros((n2, 32), dtype=np.float16)

    i = 0
    for j, (deal_str, outcome_str, auction_str, play_str) in enumerate(data_it_suit):
        d_meta = DealMeta.from_str(outcome_str)
        if d_meta.strain != "N":
            if (i != 0) and i % 10000 == 0:
                print(i)
